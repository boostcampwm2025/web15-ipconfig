...
| 대상 | 규칙 | 예시 |
|------|------|------|
| 변수/함수 | camelCase | `workspaceId`, `handleUpgrade` |
| 클래스명 | PascalCase | `WorkspaceService`, `CollaborationModule` |
| 폴더명 | camelCase | `projectSetup/` |
| 파일명 | dot notation | `workspace.service.ts`, `workspace.controller.ts`, `create-workspace.dto.ts` |
| 테스트 파일 | `__test__/` 하위 `.spec.ts` | `workspace/__test__/workspace.service.spec.ts` |
